...
public:
  ...
  particle(std::string particle_type, std::vector<double> three_momentum_input)
      : type{particle_type}, four_momentum(new std::vector<double>(4))
  {
    ...
    double energy = sqrt(pow(mass, 2) + pow(three_momentum_input.at(0), 2) +
                         pow(three_momentum_input.at(1), 2) + pow(three_momentum_input.at(2), 2));

    // Validate the energy component
    if (energy < 0 || energy > LIGHT_SPEED)
    {
